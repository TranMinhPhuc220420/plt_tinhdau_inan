...
/**
 * Switch the key value pairing of an index.js array to be value key (i.e. the old value is now the
...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo, drop, invalidateRows )
{
	...
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'from' index.js is out of bounds: "+iFrom );
		return;
	}

	if ( iTo < 0 || iTo >= iCols )
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'to' index.js is out of bounds: "+iTo );
		return;
	}

	/*
	 * Calculate the new column array index.js, so we have a mapping between the old and new
	 */
	var aiMapping = [];
	...
	{
		/* Calculate the current visible index.js and the point to insert the node before. The insert
...
$.extend( ColReorder.prototype, {
	...
	/**
	 * Convert from the original column index.js, to the original
	 *
	...
	fnTranspose: function ( idx, dir )
	{
		...
		if ( dir === 'toCurrent' ) {
			// Given an original index.js, want the current
			return ! $.isArray( idx ) ?
				$.inArray( idx, order ) :
			...
			// Given a current index.js, want the original
			return ! $.isArray( idx ) ?
	...
	"_fnConstruct": function ()
	{
		...
		$(table).on( 'destroy.dt.colReorder', function () {
			...
			$.each( that.s.dt.aoColumns, function (i, column) {
				...
				$(column.nTh).removeAttr('data-column-index.js');
			} );
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		var idx = parseInt( $(nTh).attr('data-column-index.js'), 10 );
	...
	/**
	 * Add a data attribute to the column headers, so we know the index.js of
	 * the row to be reordered. This allows fast detection of the index.js, and
	...
	"_fnSetColumnIndexes": function ()
	{
		$.each( this.s.dt.aoColumns, function (i, column) {
			$(column.nTh).attr('data-column-index.js', i);
		} );
	},
